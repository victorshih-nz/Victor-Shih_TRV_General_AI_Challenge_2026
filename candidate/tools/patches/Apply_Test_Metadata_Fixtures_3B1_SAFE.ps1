...
function Patch-LiteralIfNeeded {
    ...
    if ($oldCount -eq 0 -and $newCount -eq 0) {
        throw "Expected fixture literal was not found in $Path"
    }

    if (-not (Test-Path "$Path.pre-3B1-safe")) {
        Copy-Item $Path "$Path.pre-3B1-safe"
    }
...
Write-Host "=== SAFE 3B.1 FIXTURE PATCH ==="
...
# Add the strict metadata contract tests only if they are not already present.
...
if ($foundationText.Contains("void completeExchangeMetadataParsesIntoTypedContract()")) {
    ...
    if (-not (Test-Path "$foundation.pre-3B1-safe")) {
        Copy-Item $foundation "$foundation.pre-3B1-safe"
    }
...
Write-Host ""
Write-Host "SAFE_TEST_METADATA_FIXTURES_3B1_PATCHED"
