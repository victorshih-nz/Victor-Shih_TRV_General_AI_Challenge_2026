...
$ExpectedHead = "b342020bca0ad4d0b019ba62616f63b867f0b405"

$StageRoot = "D:\TRV_General_AI_Challenge_2026_3C_STAGE_A_b342020_v3"
...
function Get-ComposeOutput {
    ...
    return Invoke-InDirectory `
        ...
        -Command "docker" `
        -Arguments (@("compose", "-p", $ComposeProject) + $Arguments) `
        -Capture
}
...
async def main():
    ...
    def make_md_handler(label):
        async def on_md(msg):
            ...
            # PROTOCOL.md market-data format is timestamp-prefixed:
            # <ts> <type> ...
            event_type = parts[1]
            md_events[label][event_type] += 1

            # Exchange lifecycle Add:
            # <ts> A <sender:order-id> <B|S> <qty> <price>
            if (
                event_type == "A"
...
try {
    ...
    # Keep the grading entrypoint exact. docker compose therefore uses the
    # candidate directory's default project name ("candidate"). All validation
    # inspection commands explicitly target that same project with -p.
